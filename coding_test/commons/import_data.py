...
class ImportData(BaseCommand):
    ...
    def load_yield_data(self, file_path):
        """
        YieldsData will have the corn_yield updated if there is already an existing record for that year.
        """
        start_count = YieldsData.objects.all().count()
        df = pd.read_table(
            file_path,
            header=None,
        ...
        records = df.to_dict("records")
        objs = [YieldsData(**record) for record in records]
        returned_objs = YieldsData.objects.bulk_update_or_create(
        ...
        curr_count = YieldsData.objects.all().count()
        self._update_counts(records, start_count, curr_count)
